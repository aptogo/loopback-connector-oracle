...
Oracle.prototype.executeSQL = function (sql, params, options, callback) {
  ...
  function executeWithConnection(connection, autoCommit) {
    connection.execute(sql, params, {autoCommit: autoCommit, outFormat: oracledb.OBJECT}, function (err, data) {

      if (err) {
        debug('Error: %j, %s', err, sql);
      }
      ...
      connection.release(function (releaseErr) {
        ...
        if (releaseErr) {
          debug('Error releasing connection: %j', releaseErr);
  ...
  if (transaction && transaction.connection && transaction.connector === this) {
    ...
    return executeWithConnection(transaction.connection, false);
  }

  self.pool.getConnection(function (err, connection) {

    if (err) {
      debug('Error getting connection: %j', err);
      return callback(err);
    }

    executeWithConnection(connection, true);
  });
};
